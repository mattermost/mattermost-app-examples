...
def on_form_submit():
    ...
    return {
        "type": "ok",
        "text": f'Hello form got submitted. Form data: {request.json["values"]}',
    }
...
def on_install() -> dict:
    print(
        f"on_install called with payload , {request.args}, {request.data}", flush=True
    )
...
def _subscribe_team_join(context: dict) -> None:
    ...
    body = {
        "subject": "bot_joined_team",
        "call": {"path": "/bot_joined_team", "expand": {"app": "all", "team": "all"}},
    }
